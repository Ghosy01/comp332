...
class SyntaxAnalysisTests extends ParseTests {
        ...
        expression ("1 + total / 3 * sum - 5") should
            parseTo[Expression] (
                MinusExp (PlusExp (IntExp (1),
                                   StarExp (SlashExp (IdnExp (IdnUse ("total")), IntExp (3)),
                                            IdnExp (IdnUse ("sum")))),
                          IntExp (5))
          )
    }
    ...
    test ("parsing an identifier factor gives the correct tree") {
        factor ("blah") should parseTo[Expression] (IdnExp (IdnUse ("blah")))
    }
    ...
    test ("parsing a set statement gives the correct tree") {
        statement ("set bob = 11") should parseTo[Statement] (SetStmt (IdnExp (IdnUse ("bob")), IntExp (11)))
    }
    ...
    test ("parsing an expression statement gives the correct tree") {
        statement ("11 - number") should parseTo[Statement] (ExpStmt (MinusExp (IntExp (11), IdnExp (IdnUse ("number")))))
    }

    test ("parsing a program gives the correct tree") {
        parser ("set a = 3 set b = 4 + a a * a + b") should
            parseTo (
                ExpProgram (Vector (SetStmt (IdnExp (IdnUse ("a")), IntExp (3)),
                                    SetStmt (IdnExp (IdnUse ("b")), PlusExp (IntExp (4), IdnExp (IdnUse ("a")))),
                                    ExpStmt (PlusExp (StarExp (IdnExp (IdnUse ("a")), IdnExp (IdnUse ("a"))), IdnExp (IdnUse ("b"))))))
            )
    }
    ...
    test ("simple expression in conditional and simpl expression statement in branch") {
        statement ("if (a) { a }") should parseTo[Statement] (
            IfStmt (IdnExp (IdnUse ("a")), Vector (ExpStmt (IdnExp (IdnUse ("a")))))
        )
    }

    test ("more complex conditional, multiple statements in branch") {
        statement ("if (a + b) { set b = 0 b * a }") should parseTo[Statement] (
            IfStmt (PlusExp (IdnExp (IdnUse ("a")), IdnExp (IdnUse ("b"))),
                Vector (SetStmt (IdnExp (IdnUse ("b")), IntExp (0)),
                      ExpStmt (StarExp (IdnExp (IdnUse ("b")), IdnExp (IdnUse ("a"))))))
        )
    }

    test ("nested conditionals are parsed correctly") {
        statement ("if (a + 1) { if (b * 2) { set a = b - 3 }}") should parseTo[Statement] (
            IfStmt (PlusExp (IdnExp (IdnUse ("a")), IntExp (1)),
                Vector (IfStmt (StarExp (IdnExp (IdnUse ("b")), IntExp (2)),
                          Vector (SetStmt (IdnExp (IdnUse ("a")), MinusExp (IdnExp (IdnUse ("b")), IntExp (3)))))))
        )
    }
