...
/**
 * Compile the expression language program in the file given as the
 * first command-line argument to stack code and process the source tree
 * to describe whether expression statements contain constant statements
 * or not.
 */
object Main {
    ...
    def process (program : ExpProgram, messaging : Messaging) {
        ...
        if (messages.length > 0)
            ...
            val targettree = translate (program)
            println (layout (any (targettree)))

            // Execute the target tree
            execute (targettree)
        }
    }
